...
describe('GameGateway', () => {
    ...
    describe('abandonMultiplayerRoom', () => {
        ...
        it('should call leave with second room in socket.rooms', () => {
            ...
            jest.spyOn<any, string>(gateway, 'winMultiplayerClassicGame').mockImplementation(() => {
                return;
            });
            gateway['rooms'] = [
                {
        ...
        it('should call winMultiplayerClassicGame', () => {
    ...
    describe('winMultiplayerClassicGame', () => {
        it('should call leaveGameRoom if a socketId is passed', async () => {
            ...
            expect(spy).toHaveBeenCalledWith(
                ...
                { socketId: 'blabla', name: 'simon' },
                { ranking: undefined, time: undefined },
            );
        });
        ...
        it('should call removeAndLeaveRoom once if no socketId is passed', async () => {
            ...
            expect(spy).toHaveBeenCalledWith(GameEvents.PlayerWonGame, { socketId: 'simon', name: 'simon' }, { ranking: undefined, time: undefined });
        });
        ...
        it('should emit the event on the room', async () => {
            ...
            expect(spy).toHaveBeenCalledWith(GameEvents.PlayerWonGame, { socketId: 'simon', name: 'name' }, { ranking: undefined, time: undefined });
        });
    });
    ...
    describe('handleDisconnect', () => {
        ...
        it('should call winMultiplayerClassic', () => {
            gateway['pendingGames'] = [];
            gateway['rooms'] = [{ gameId: 0, hostId: 'simon', roomNumber: 0, stringFormat: 'room' }];
            ...
            jest.spyOn(gateway, 'leaveGameRoom').mockImplementation();
            const spy = jest.spyOn<any, any>(gateway, 'winMultiplayerClassicGame').mockImplementation();
            gateway.handleDisconnect(socket);
            expect(spy).toHaveBeenCalled();
        });
        it('should broadcast participant left if player is not host of a waiting room but isnot in a room', () => {
            gateway['pendingGames'] = [];
            gateway['rooms'] = [];
            Object.assign(socket, { id: 'simon' });
            const bo = createStubInstance(BroadcastOperator);
