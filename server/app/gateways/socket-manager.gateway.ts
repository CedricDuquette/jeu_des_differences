...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
    ...
    createSoloGame(socket: Socket, gameId: number): void {
        ...
        this.rooms.push(roomId);
        this.gameManager.createGame(roomId, this.gameService.getGame(gameId).differenceImage.length, true);
    }
    ...
    abandonMultiplayerRoom(socket: Socket): void {
        ...
        if (this.rooms.find((room) => room.stringFormat === roomName)) {
            this.winMultiplayerClassicGame(socket, player.name, player.socketId);
        }
    ...
    startGame(socket: Socket): void {
        this.gameManager.findGameRoom(socket.id).gameStarted = true;
    }
    ...
    private async winMultiplayerClassicGame(socket: Socket, playerName: string, socketId?: string): Promise<void> {
        ...
        if (this.isNewRecord(winnerRanking)) this.sendGlobalMessage(this.gameManager.getNewHighScoreMessage(socket.id, false, winnerRanking));
        this.server.emit(GameEvents.ModifiedDatabase);
        const roomName: string = [...socket.rooms][1];
        if (this.rooms.find((room) => room.stringFormat === roomName)) {
            this.server
                ...
                .emit(
                    ...
                    { ranking: winnerRanking, time: this.gameManager.getClockTime(socket.id) },
                );
        }
    }
    ...
    private async manageGameEnd(socket: Socket, gameRoom: GameRoom): Promise<void> {
        ...
        if (this.gameManager.checkForSoloWin(gameRoom)) {
            const winnerRanking: PlayerRanking = await this.gameManager.getWinnerRanking(socket.id);
            if (this.isNewRecord(winnerRanking)) this.sendGlobalMessage(this.gameManager.getNewHighScoreMessage(socket.id, true, winnerRanking));
